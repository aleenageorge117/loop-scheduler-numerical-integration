...
void* iterationIntegration(void* p) {
...
struct arg_structure getObjStruct(float a,float b,int funct_id,int intensity,float mid,string sync,int n) {
  struct arg_structure obj;
  obj.a = a;
  obj.b = b;
  obj.funct_id = funct_id;
...
int main (int argc, char* argv[]) {
  ...
  if(0 == sync.compare("iteration")) {
    for(int i= 0; i<nbthreads;i++) {
      obj_struct[i] = getObjStruct(a,b,funct_id,intensity,mid,sync,n);
      pthread_create(&thread[i], NULL, iterationIntegration, (void*) &obj_struct[i]);
    }   
  } else if(0 == sync.compare("thread")) {
    for(int i= 0; i<nbthreads;i++) {
      obj_struct[i] = getObjStruct(a,b,funct_id,intensity,mid,sync,n);
      pthread_create(&thread[i], NULL, threadIntegration,(void*) &obj_struct[i]);
    }
  }
  else if(0 == sync.compare("chunk")) {
    for(int i= 0; i<nbthreads;i++) {
      obj_struct[i] = getObjStruct(a,b,funct_id,intensity,mid,sync,n);
      pthread_create(&thread[i], NULL, chunkIntegration,(void*) &obj_struct[i]);
    }
  }
